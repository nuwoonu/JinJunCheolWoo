 ...
 *
 * 발표용 데이터 세팅:
 *  - 가거도초등학교 (id=9523): 1~6학년 각 2반 (12학급), 학생 24명, 교사 12명, 교직원 5명
 *  - 가경중학교     (id=7490): 1~3학년 각 3반 (9학급),  학생 27명, 교사 10명, 교직원 5명
...
public class TestDataService {
    ...
    public Map<String, Object> seedAll() {
        ...
        School elemSchool = schoolRepository.findById(ELEM_SCHOOL_ID)
                .orElseThrow(() -> new IllegalStateException(
                        "가거도초등학교(id=" + ELEM_SCHOOL_ID + ")를 DB에서 찾을 수 없습니다. 학교 데이터를 먼저 확인하세요."));
        ...
        // ── 공지사항 & 알림장 ─────────────────────────────────────────────────
        if (!elemTeachers.isEmpty()) seedNotices(elemSchool, elemTeachers.get(0).getUser(), elemClasses, elemTeachers);
        if (!midTeachers.isEmpty())  seedNotices(midSchool,  midTeachers.get(0).getUser(),  midClasses,  midTeachers);
    ...
    private void seedCalendar(School school, int year) {
        ...
        // 1학기
        events.add(buildEvent("입학식 및 개학식",           LocalDate.of(year, 3,  2), null,                      EventType.ACADEMIC, null, "새 학년도 시작"));
        events.add(buildEvent("1학기 중간고사",             LocalDate.of(year, 4, 13), LocalDate.of(year, 4, 15), EventType.EXAM,     null, "1학기 중간고사 기간"));
        ...
        events.add(buildEvent("1학기 기말고사",             LocalDate.of(year, 6, 22), LocalDate.of(year, 6, 24), EventType.EXAM,     null, "1학기 기말고사 기간"));
        events.add(buildEvent("1학기 종업식",               LocalDate.of(year, 7, 16), null,                      EventType.ACADEMIC, null, null));
        events.add(buildEvent("여름방학",                   LocalDate.of(year, 7, 17), LocalDate.of(year, 8, 16), EventType.VACATION, null, null));

        // 2학기
        events.add(buildEvent("2학기 개학",                 LocalDate.of(year, 8, 17), null,                      EventType.ACADEMIC, null, null));
        events.add(buildEvent("2학기 중간고사",             LocalDate.of(year, 9, 28), LocalDate.of(year, 9, 30), EventType.EXAM,     null, "2학기 중간고사 기간"));
        events.add(buildEvent("추석 연휴",                  LocalDate.of(year,10,  2), LocalDate.of(year,10,  4), EventType.HOLIDAY,  null, null));
        ...
        events.add(buildEvent("2학기 기말고사",             LocalDate.of(year,12,  7), LocalDate.of(year,12,  9), EventType.EXAM,     null, "2학기 기말고사 기간"));
        events.add(buildEvent("2학기 종업식",               LocalDate.of(year,12, 24), null,                      EventType.ACADEMIC, null, null));
        events.add(buildEvent("겨울방학",                   LocalDate.of(year,12, 25), LocalDate.of(year+1, 2, 9), EventType.VACATION, null, null));
        events.add(buildEvent("졸업식",                     LocalDate.of(year+1, 2, 10), null,                    EventType.ACADEMIC, null, null));

        events.forEach(e -> { e.setSchool(school); calendarRepository.save(e); });
    }
            ...
            String[][] notices = {
                {
                    ...
                    "<p>안녕하세요. <strong>" + school.getName() + "</strong> 학부모님께 학교 운영 계획을 안내드립니다.</p>" +
    ...
    private void seedClassGoals(List<Classroom> classrooms, int year) {
        ...
        for (int i = 0; i < classrooms.size(); i++) {
            Classroom c = classrooms.get(i);
            if (classGoalRepository.findByClassroom_CidAndYearAndMonth(c.getCid(), year, month).isPresent()) {
                continue;
            }
