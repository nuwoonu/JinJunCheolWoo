...
public class BehaviorRecordService {
    ...
    // 등록 또는 수정 (학년+학기 기준 upsert)
    @Transactional
    public BehaviorRecordDTO save(Long studentId, BehaviorRecordRequestDTO request) {
        log.info("행동 특성 저장 - 학생 ID: {}, 학년: {}, 학기: {}", studentId, request.getYear(), request.getSemester());
        ...
        BehaviorRecord record = behaviorRecordRepository
                .findByStudentIdAndYearAndSemester(studentId, request.getYear(), request.getSemester())
                .map(existing -> {
