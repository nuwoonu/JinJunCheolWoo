...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.startsWith("/api/") || url.pathname.startsWith("/@") || url.pathname.startsWith("/node_modules/"))
    return;

  event.respondWith(
    fetch(event.request).catch(() =>
      caches.match(event.request).then((cached) => cached || new Response("Service Unavailable", { status: 503 })),
    ),
  );
});
...
self.addEventListener("notificationclick", (event) => {
  ...
  const actionUrl =
    event.notification.data && event.notification.data.actionUrl ? event.notification.data.actionUrl : "/hub";
