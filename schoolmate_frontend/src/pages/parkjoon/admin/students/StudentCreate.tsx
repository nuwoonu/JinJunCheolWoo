...
export default function StudentCreate() {
  ...
  const [form, setForm] = useState({ name: '', code: '', email: '', password: '', classroomId: '' })
  ...
  useEffect(() => {
    admin.get('/students/classrooms').then(r => setClassrooms(r.data)).catch(() => {})
  }, [])
    ...
    <AdminLayout>
      <div className="breadcrumb d-flex align-items-center gap-3 mb-24">
        ...
        <div>
          ...
          <p className="text-neutral-600 mt-4 mb-0">새 학생 계정을 등록합니다.</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="card">
        <div className="card-body p-4">
          <h5 className="mb-4 text-primary fw-bold"><i className="bi bi-person-circle me-2" />기본 인적 사항</h5>
          <div className="row g-3 mb-5">
            <div className="col-md-6">
              <label className="form-label fw-bold">이름 <span className="text-danger">*</span></label>
            ...
            <div className="col-md-12">
              <label className="form-label fw-bold">이메일 (ID) <span className="text-danger">*</span></label>
            ...
            <div className="col-md-12">
              <label className="form-label fw-bold">초기 비밀번호 <span className="text-danger">*</span></label>
              <input type="password" className="form-control" required value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} />
            </div>
          </div>

          <h5 className="mb-4 text-primary fw-bold"><i className="bi bi-journal-check me-2" />최초 학급 배정 (선택)</h5>
          <div className="row g-3 mb-5">
            <div className="col-md-12">
          ...
          <h5 className="mb-4 text-primary fw-bold"><i className="bi bi-people me-2" />보호자 연동 (선택)</h5>
          <div className="mb-3">
            ...
            <div className="border rounded p-2 bg-light" style={{ minHeight: 50 }}>
              {guardians.length === 0
                ? <p className="text-muted small text-center mb-0 py-2">연동할 보호자를 검색하여 추가하세요.</p>
                : guardians.map(g => (
                  <span key={g.parentId} className="badge bg-primary me-2 mb-1">
                    {g.name} ({RELATION_LABEL[g.relation] ?? g.relation})
                    <button type="button" className="btn-close btn-close-white ms-1" style={{ fontSize: '0.5rem' }} onClick={() => setGuardians(prev => prev.filter(x => x.parentId !== g.parentId))} />
                  </span>
                ))
              }
            </div>
