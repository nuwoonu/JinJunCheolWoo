...
export default function NoticeList() {
  ...
  const load = (p = 0, kw = keyword) =>
    admin.get('/notices', { params: { keyword: kw || undefined, page: p, size: 15 } })
      ...
      <div className="card shadow-sm border-0">
        <div className="card-header bg-white py-3">
          <div className="row align-items-center">
            <div className="col"><h5 className="mb-0 text-dark">공지사항 목록</h5></div>
            <div className="col-auto">
              <form className="input-group input-group-sm" onSubmit={search}>
                <input className="form-control" style={{ width: 300 }} placeholder="제목 검색..." value={keyword} onChange={e => setKeyword(e.target.value)} />
        ...
        <div className="card-body p-0">
          <table className="table table-hover align-middle mb-0">
            <thead className="table-light">
              <tr>
                <th className="text-center" style={{ width: 80 }}>No</th>
                <th>제목</th>
                <th className="text-center" style={{ width: 120 }}>작성자</th>
                <th className="text-center" style={{ width: 120 }}>작성일</th>
            ...
            <tbody>
              {list.map((n: any) => (
                <tr key={n.id}>
                  ...
                  <td>
                    <Link to={`${BASE}/notices/${n.id}`} className="fw-bold text-decoration-none text-dark">
                      {n.title}
                    </Link>
                  </td>
                  <td className="text-center text-muted">{n.writerName}</td>
                  <td className="text-center text-muted">{n.createdDate?.split('T')[0]}</td>
                  <td className="text-center text-muted">{n.viewCount}</td>
                </tr>
              ))}
              {list.length === 0 && <tr><td colSpan={5} className="text-center py-5 text-muted">공지사항이 없습니다.</td></tr>}
            </tbody>
          </table>
        </div>
