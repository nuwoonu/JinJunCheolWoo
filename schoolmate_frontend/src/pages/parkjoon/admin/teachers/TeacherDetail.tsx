...
export default function TeacherDetail() {
  ...
  const [form, setForm] = useState({ name: '', email: '', code: '', subject: '', department: '', position: '', statusName: '' })
  ...
  const load = () =>
    admin.get(`/teachers/${uid}`).then(r => {
      setTeacher(r.data)
      const d = r.data
      setForm({ name: d.name ?? '', email: d.email ?? '', code: d.code ?? '', subject: d.subject ?? '', department: d.department ?? '', position: d.position ?? '', statusName: d.statusName ?? '' })
    })
  ...
  const addRole = async () => {
    ...
    await admin.post(`/teachers/${uid}/role`, null, { params: { role: newRole } })
    setNewRole('')
    load()
  }
  ...
  if (!teacher) return <AdminLayout><div className="text-center py-5"><div className="spinner-border" /></div></AdminLayout>
  ...
  return (
    <AdminLayout>
      <div className="breadcrumb d-flex align-items-center gap-3 mb-24">
        <button type="button" onClick={() => navigate(ADMIN_ROUTES.TEACHERS.LIST)} style={{ background: "none", border: "1px solid #e5e7eb", borderRadius: 6, padding: "4px 10px", cursor: "pointer", color: "#6b7280" }}>
            ...
            <div className="card-body">
              ...
              <div className="text-start px-2">
                ...
                <div className="mb-2">
                  ...
                  <span className="fw-semibold">{teacher.department ?? '-'} / {teacher.position ?? '-'}</span>
                </div>
                <div>
                  <small className="text-muted d-block">담당 과목</small>
                  <span className="badge rounded-pill bg-light text-dark border">{teacher.subject ?? '-'}</span>
                </div>
              </div>
            </div>
          </div>
