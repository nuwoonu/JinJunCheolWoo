  ...
  const results = await Promise.allSettled(
    statuses.map((s) =>
      admin
        ...
        .then(
          (r) =>
            [s, (r.data.totalElements as number | undefined) ?? 0] as const,
        )
        .catch(() => [s, 0] as const),
    ),
  );
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetchStatusCounts(
      "teachers",
      ["EMPLOYED", "LEAVE", "RETIRED"],
      schoolId,
    ).then(setTeacherCounts);
    fetchStatusCounts(
      "staffs",
      ["EMPLOYED", "LEAVE", "DISPATCHED", "RETIRED"],
      schoolId,
    ).then(setStaffCounts);
  ...
  useEffect(() => {
    ...
    admin
      .get(
        `/students?roleRequestStatus=${studentRRFilter}&size=5${ampSchoolParam}`,
      )
      .then((r) => {
  ...
  useEffect(() => {
    ...
    admin
      .get(
        `/teachers?roleRequestStatus=${teacherRRFilter}&size=5${ampSchoolParam}`,
      )
      .then((r) => {
        const content = r.data.content ?? [];
        setPendingTeachers(
  ...
  const TAB_CONFIGS = {
    STUDENT: {
      ...
      chartData: gradeKeys.map((gr) => ({
        name: `${gr}학년`,
        value: classesByGrade[gr].students,
      })),
      chartTitle: "학년별 분포",
    },
    TEACHER: {
  ...
  const activeRate =
    currentTab.total > 0
      ? Math.round((currentTab.activeCount / currentTab.total) * 100)
      : 0;
  const inactiveRate =
    currentTab.total > 0
      ? Math.round((currentTab.inactiveCount / currentTab.total) * 100)
      : 0;
            ...
            >
              ...
              >
                ...
                <i
                  ...
                  style={{ fontSize: 16, color: "#25A194" }}
                />
              </div>
              <p
                style={{
                    ...
                    >
                      <div
                        style={{
                          ...
                          borderRadius: "50%",
                          background:
                            "#25a1942e" /* [soojin] 사이드바 학생 프로필 바탕색(bg-primary-100) 적용 */,
                          color: "#25A194",
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
            ...
            <div style={{ padding: "20px 20px 20px" }}>
              {gradeKeys.length === 0 ? (
                ...
                <>
                  ...
                  {/* 전체 합계 */}
                  {(() => {
                    const totalClasses = gradeKeys.reduce(
                      (sum, gr) => sum + classesByGrade[gr].count,
                      0,
                    );
                    const totalStudentsAll = gradeKeys.reduce(
                      (sum, gr) => sum + classesByGrade[gr].students,
                      0,
                    );
                    return (
