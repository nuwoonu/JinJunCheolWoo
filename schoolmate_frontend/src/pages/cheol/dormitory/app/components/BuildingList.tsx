...
export default function BuildingList() {
  ...
  const [newBuilding, setNewBuilding] = useState({ name: "", floors: 5, roomsPerFloor: 4, bedsPerRoom: 2 });
  ...
  const handleAddBuilding = async () => {
    ...
    setAdding(true);
    try {
      await addBuilding(newBuilding.name.trim(), newBuilding.floors, newBuilding.roomsPerFloor, newBuilding.bedsPerRoom);
      setShowAddForm(false);
      setNewBuilding({ name: "", floors: 5, roomsPerFloor: 4, bedsPerRoom: 2 });
      refetch();
    } catch (e: any) {
    ...
    >
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        {!isStudent && showAddForm && ( // cheol
          ...
          >
            <h2 style={{ fontSize: "16px", fontWeight: 600, margin: "0 0 16px", color: "#1e293b" }}>새 건물 추가</h2>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "12px", marginBottom: "16px" }}>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewBuilding((v) => ({ ...v, floors: Number(e.target.value) }))}
                  style={{ width: "100%", padding: "8px 10px", border: "1px solid #e2e8f0", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box", outline: "none" }}
                />
              </div>
              <div>
                <label style={{ fontSize: "12px", color: "#64748b", display: "block", marginBottom: "4px" }}>층당 호실 수</label>
                <input
                  type="number" min={1}
                  value={newBuilding.roomsPerFloor}
                  onChange={(e) => setNewBuilding((v) => ({ ...v, roomsPerFloor: Number(e.target.value) }))}
                  style={{ width: "100%", padding: "8px 10px", border: "1px solid #e2e8f0", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box", outline: "none" }}
                />
              </div>
              <div>
