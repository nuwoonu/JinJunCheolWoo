    ...
    <DashboardLayout>
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
          ...
          <p className="text-neutral-600 mt-4 mb-0">학생 리스트</p>
        </div>
            ...
            <table className="table bordered-table mb-0">
              <thead>
                <tr>
                  ...
                  <th scope="col">연락처</th>
                  <th scope="col">이메일</th>
                  <th scope="col" className="text-center">성별</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="text-center py-24 text-secondary-light">등록된 학생이 없습니다.</td></tr>
                ) : (
                  filtered.map(s => (
                    <tr key={s.id}>
            ...
            <div className="modal-content radius-12">
              ...
              <div className="modal-body p-24">
                <div className="text-center mb-24">
                  ...
                  <span className="text-secondary-light">{selectedStudent.fullStudentNumber ?? selectedStudent.studentCode ?? '-'}</span>
                </div>
                <div className="d-flex flex-column gap-12">
                  {[
                    { label: '학년/반', value: selectedStudent.year && selectedStudent.classNum ? `${selectedStudent.year}학년 ${selectedStudent.classNum}반` : '-' },
                    { label: '연락처', value: selectedStudent.phone },
                    { label: '이메일', value: selectedStudent.userEmail },
                    { label: '성별', value: selectedStudent.gender === 'MALE' ? '남' : selectedStudent.gender === 'FEMALE' ? '여' : '-' },
                    // [woo] STATUS_LABEL로 한글 변환, 매핑 없는 값은 원본 그대로 표시
                    { label: '상태', value: selectedStudent.status ? (STATUS_LABEL[selectedStudent.status] ?? selectedStudent.status) : null },
                  ].map(row => (
