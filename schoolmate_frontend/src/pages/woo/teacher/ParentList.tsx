...
export default function ParentList() {
  ...
  const handleRegister = () => {
    if (!registerForm.studentInfoId) { alert('학생을 선택해주세요.'); return }
    if (!registerForm.parentName.trim()) { alert('학부모 이름을 입력해주세요.'); return }
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, gap: 12, flexWrap: 'wrap', flexShrink: 0 }}>
          ...
          >
            ...
            <button
              ...
              type="button"
              onClick={() => { setType(''); setKeyword(''); load(0, '', '') }}
            >초기화</button>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto', minHeight: 0 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
              <colgroup>
                {/* [soojin] 학생 열 auto로 인해 1·2열 간격 과다 → 퍼센트 비율로 전환해 균형 조정 */}
                <col style={{ width: '18%' }} />
                <col style={{ width: '15%' }} />
                <col style={{ width: '25%' }} />
                <col style={{ width: '15%' }} />
              ...
              <tbody>
                {list.length === 0 ? (
                  ...
                  <tr key={p.id}>
                    <td style={{ padding: '14px 16px', fontSize: 13, color: '#374151', borderBottom: '1px solid #f3f4f6', verticalAlign: 'middle', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    ...
                    <td style={{ padding: '14px 16px', fontSize: 13, color: '#6b7280', borderBottom: '1px solid #f3f4f6', verticalAlign: 'middle', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.email ?? '-'}</td>
                    <td style={{ padding: '14px 16px', fontSize: 13, color: '#6b7280', borderBottom: '1px solid #f3f4f6', verticalAlign: 'middle', whiteSpace: 'nowrap' }}>{p.phone ?? '-'}</td>
                    <td style={{ padding: '14px 16px', fontSize: 13, borderBottom: '1px solid #f3f4f6', verticalAlign: 'middle' }}>
                      {p.linked
                        ? <span style={{ padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: 'rgba(22,163,74,0.1)', color: '#16a34a' }}>연동됨</span>
                        : <span style={{ padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: '#f3f4f6', color: '#6b7280' }}>미연동</span>
                      }
                    </td>
                    <td style={{ padding: '14px 16px', fontSize: 13, borderBottom: '1px solid #f3f4f6', verticalAlign: 'middle' }}>
                      {p.statusName === 'ACTIVE'
                        ? <span style={{ padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: 'rgba(22,163,74,0.1)', color: '#16a34a' }}>{p.statusName}</span>
                        : <span style={{ padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: '#f3f4f6', color: '#6b7280' }}>{p.statusName ?? '-'}</span>
                      }
                    </td>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content radius-12">
              ...
              <div className="modal-footer border-top py-16 px-24">
                <button type="button" className="btn btn-outline-neutral-300 radius-8" onClick={() => setSelectedParent(null)}>닫기</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content radius-12">
                ...
                {registerResult ? (
                  <div className="text-center">
                    ...
                    <div className="bg-neutral-50 radius-8 p-16 text-start">
                      {[
                        ...
                        <div key={row.label} className={`d-flex justify-content-between py-8 ${i < arr.length - 1 ? 'border-bottom' : ''}`}>
                          <span className="text-secondary-light text-sm">{row.label}</span>
                          <span className={`fw-semibold text-sm ${row.highlight ? 'text-primary-600' : row.warn ? 'text-warning-600' : ''}`}>{row.value}</span>
                        </div>
                      ))}
                    </div>
                    <p className="text-secondary-light text-xs mt-12">학부모에게 로그인 ID와 초기 비밀번호를 안내해주세요.</p>
                  </div>
                ) : (
                  <div className="d-flex flex-column gap-16">
                    ...
                    <div>
                      ...
                      <select className="form-select radius-8" value={registerForm.studentInfoId} onChange={e => setRegisterForm(f => ({ ...f, studentInfoId: Number(e.target.value) }))}>
                        <option value={0}>-- 학생을 선택하세요 --</option>
                        {students.map(s => (
                          <option key={s.studentId} value={s.studentId}>{s.studentNumber ? `${s.studentNumber}번 ` : ''}{s.name}</option>
                        ))}
                      </select>
                    </div>
                    <div>
                      <label className="form-label fw-medium text-sm mb-6">학부모 이름 <span className="text-danger-600">*</span></label>
                      <input type="text" className="form-control radius-8" placeholder="학부모 이름 입력" value={registerForm.parentName} onChange={e => setRegisterForm(f => ({ ...f, parentName: e.target.value }))} />
                    </div>
                    <div>
                      <label className="form-label fw-medium text-sm mb-6">전화번호 <span className="text-danger-600">*</span></label>
                      <input type="tel" className="form-control radius-8" placeholder="01012345678" value={registerForm.phoneNumber} onChange={e => setRegisterForm(f => ({ ...f, phoneNumber: e.target.value }))} />
                    </div>
                    <div>
                      <label className="form-label fw-medium text-sm mb-6">관계</label>
                      <select className="form-select radius-8" value={registerForm.relationship} onChange={e => setRegisterForm(f => ({ ...f, relationship: e.target.value }))}>
              ...
              <div className="modal-footer border-top py-16 px-24 gap-8">
                {registerResult ? (
                  <button type="button" className="btn btn-primary-600 radius-8" onClick={() => { setRegisterResult(null); setRegisterForm({ studentInfoId: 0, parentName: '', phoneNumber: '', relationship: 'MOTHER' }) }}>
                    추가 등록
                  </button>
                ) : (
                  <button type="button" className="btn btn-success-600 radius-8 d-flex align-items-center gap-6" onClick={handleRegister} disabled={registerLoading}>
                    {registerLoading ? <><span className="spinner-border spinner-border-sm" />등록 중...</> : <><iconify-icon icon="mdi:check" />등록하기</>}
                  </button>
                )}
                <button type="button" className="btn btn-outline-neutral-300 radius-8" onClick={() => setShowRegisterModal(false)}>닫기</button>
              </div>
            </div>
          </div>
        </div>
