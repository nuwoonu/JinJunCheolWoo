...
export default function ParentHomework() {
  ...
  useEffect(() => {
    ...
    setDataLoading(true)
    Promise.all([
      api.get(`/homework/parent/${selectedId}`).then(res => res.data ?? []).catch(() => []),
      api.get(`/quiz/parent/${selectedId}`).then(res => res.data ?? []).catch(() => []),
    ]).then(([hw, qz]) => {
      setHomeworks(hw)
      setQuizzes(qz)
        ...
        <div className="d-flex align-items-center gap-8 mb-24">
          ...
          {children.map(c => (
            ...
            >
              {c.name}
              {c.grade && c.classNum && (
                <span className="ms-4 text-xs opacity-75">({c.grade}학년 {c.classNum}반)</span>
              )}
            </button>
          ))}
        </div>
      ...
      {dataLoading ? (
        ...
        <div className="card radius-12">
          <div className="card-header py-16 px-24 border-bottom">
            <h6 className="mb-0">
              {selectedChild ? `${selectedChild.name}의 과제 목록` : '과제 목록'}
            </h6>
          </div>
          {homeworks.length === 0 ? (
                    ...
                    {homeworks.map(hw => {
                      ...
                      return (
                        <tr key={hw.id}>
                          ...
                          <td><span className={`badge ${hwSt.cls}`}>{hwSt.text}</span></td>
                          <td>
                            {hw.submitted == null ? (
        ...
        <div className="card radius-12">
          <div className="card-header py-16 px-24 border-bottom">
            <h6 className="mb-0">
              {selectedChild ? `${selectedChild.name}의 퀴즈 목록` : '퀴즈 목록'}
            </h6>
          </div>
          {quizzes.length === 0 ? (
                    ...
                    {quizzes.map(qz => {
                      ...
                      return (
                        <tr key={qz.id}>
                          ...
                          <td>
                            <span className={`badge ${isOpen ? 'bg-success-100 text-success-600' : 'bg-danger-100 text-danger-600'}`}>
                              {isOpen ? '진행중' : '마감'}
                            </span>
                          </td>
                          <td className="text-secondary-light">
                            {qz.myAttemptCount != null ? (
                              <span>
                                {qz.myAttemptCount}회
                                {qz.maxAttempts && <span className="text-xs ms-2 opacity-75">/ {qz.maxAttempts}회</span>}
                              </span>
                            ) : '-'}
                          </td>
                          <td>
