...
export default function QuizEdit() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [quizRes, classroomRes] = await Promise.all([api.get(`/quiz/${id}`), api.get("/homework/classrooms")]);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="d-flex justify-content-end gap-8 mb-24">
        <button type="button" className="btn btn-outline-neutral-300 radius-8" onClick={() => navigate(`/quiz/${id}`)}>
          취소
        </button>
