      ...
      {loading ? (
        ...
        <div className="d-flex flex-column gap-0" style={{ overflowY: 'auto', maxHeight: 240 }}>
          {students.map((s, i) => (
            ...
            >
              {/* 번호 */}
              <span className="text-secondary-light text-md me-12" style={{ minWidth: 16, textAlign: 'right' }}>
                {s.studentNumber ?? i + 1}
              </span>
              ...
              <div className="d-flex align-items-center gap-6 flex-grow-1">
                <span className="text-md fw-medium text-dark">{s.userName}</span>
                {s.role === 'CLASS_PRESIDENT' && (
                  <span
                    className="text-md fw-semibold px-6 py-2 rounded"
                    style={{ background: '#ff6b35', color: '#fff', fontSize: 13 }}
                  >
                    반장
                  </span>
                )}
                {s.role === 'VICE_PRESIDENT' && (
                  <span
                    className="text-md fw-semibold px-6 py-2 rounded"
                    style={{ background: '#f59e0b', color: '#fff', fontSize: 13 }}
                  >
                    부반장
                  </span>
                )}
              ...
              {(() => {
                ...
                return (
                  <span
                    className="text-md fw-semibold px-8 py-3 rounded"
                    style={{ background: cfg.color, color: '#fff', fontSize: 13, minWidth: 36, textAlign: 'center' }}
                  >
                    {cfg.label}
                  </span>
                )
