...
// [soojin] /board/parent/:id - 학부모 게시판 상세 (좋아요/북마크/댓글/태그 UI 포함)
...
function AvatarIcon({ name }: { name: string }) {
  return (
    <div
      ...
      style={{ width: 36, height: 36, fontSize: 14, background: '#f97316' }}
    >
      {name ? name.charAt(0) : '?'}
    </div>
  )
...
export default function ParentBoardDetail() {
  ...
  const handleCommentSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      await api.post(`/board/${id}/comments`, { content: newComment.trim(), parentId: replyTo?.id ?? null })
      setNewComment('')
      setReplyTo(null)
  ...
  const attachments: { name: string; url: string; size?: number }[] =
    board.attachments && board.attachments.length > 0
      ? board.attachments.map(a => ({ name: a.originalName, url: `/api/board/file/${a.storedName}`, size: a.fileSize }))
      : board.attachmentUrl
  ...
  return (
    <DashboardLayout>
      ...
      <div className="d-flex align-items-center gap-8 mb-24">
        <Link to="/board/parent" className="w-36-px h-36-px rounded-circle bg-neutral-100 d-flex align-items-center justify-content-center text-secondary-light">
          <i className="ri-arrow-left-line text-lg" />
        </Link>
        <h5 className="fw-bold mb-0">학부모 게시판</h5>
      </div>
      ...
      <div className="card radius-12" style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <div className="card-header py-20 px-24 border-bottom">
          <div className="d-flex align-items-center gap-8 mb-10">
            <span className="badge bg-warning-100 text-warning-600 fw-medium" style={{ fontSize: 12 }}>학부모 게시판</span>
        ...
        <div className="card-body px-24 py-20">
          {board.content.includes('<') ? (
            ...
            <div style={{ fontSize: 15, lineHeight: 2, color: '#334155', whiteSpace: 'pre-wrap', minHeight: 120 }}>{board.content}</div>
          )}
        </div>
        ...
        <div className="px-24 py-16 border-top d-flex align-items-center justify-content-between flex-wrap gap-8">
          ...
          <div className="d-flex align-items-center gap-8 flex-wrap">
            ...
            <Link to="/board/parent" className="btn btn-outline-neutral-300 btn-sm radius-8 d-flex align-items-center gap-6" style={{ fontSize: 13 }}>
