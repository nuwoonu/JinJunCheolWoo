...
export default function ParentBoard() {
  ...
  // [soojin] 학부모: 선택된 자녀 uid — fetchBoards에 전달
  ...
  const fetchBoards = (p = 0, kw = keyword, st = searchType, sb = sortBy, tg = selectedTag) => {
    ...
    if (selectedChildId) params.set("studentUserUid", selectedChildId);
    api
      .get(`/board/parent-board?${params}`)
      .then((res) => {
  ...
  const fetchSidebar = () => {
    ...
    Promise.all([
      api.get("/board/stats?boardType=PARENT_BOARD"),
      api.get("/board/popular?boardType=PARENT_BOARD&limit=5"),
    ])
                  ...
                  {boards.map((board) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => navigate(`/board/parent/${board.id}`)}
