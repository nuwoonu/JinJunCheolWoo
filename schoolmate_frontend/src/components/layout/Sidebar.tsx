...
export default function Sidebar() {
  ...
  const ROLE_LABEL: Record<string, string> = { STUDENT: '학생', TEACHER: '선생님', PARENT: '학부모', ADMIN: '관리자' }
    ...
    <aside className={`sidebar${isOpen ? ' sidebar-open' : ''}${isCollapsed ? ' active' : ''}`}>
      ...
      <div>
        <div className="sidebar-logo d-flex align-items-center justify-content-between">
          ...
          <button type="button" className="text-xxl d-xl-flex d-none line-height-1 sidebar-toggle text-neutral-500" aria-label="Collapse Sidebar" onClick={toggleCollapse}>
          ...
          <div className="dropdown profile-dropdown" ref={profile.ref}>
            ...
            >
              <span className="d-flex align-items-start gap-10">
                <img src="/images/thumbs/leave-request-img2.png" alt="Thumbnail" className="w-40-px h-40-px rounded-circle object-fit-cover flex-shrink-0" />
            ...
            <ul className={`dropdown-menu dropdown-menu-lg-end border p-12${profile.isOpen ? ' show' : ''}`}>
              <li><Link to="/user/profile" className="dropdown-item rounded text-secondary-light bg-hover-neutral-200 text-hover-neutral-900 d-flex align-items-center gap-2 py-6"><i className="ri-user-3-line" /> 나의 프로필</Link></li>
              <li>
                <button
                  type="button"
        ...
        <ul className="sidebar-menu" id="sidebar-menu">
          ...
          {user?.authenticated && (() => {
            const dashboardPath =
              ...
              role === 'PARENT'  ? '/parent/dashboard'  : '/main'
            return <li><a href={dashboardPath}><i className="ri-home-4-line" /><span>홈</span></a></li>
          })()}
          ...
          {has('TEACHER') && (
            <li className={dc(open.myclass)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('myclass') }}>
                <i className="ri-team-line" /><span>나의 학급</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/teacher/myclass"><i className="ri-circle-fill circle-icon w-auto" /> 학급 현황</Link></li>
                <li><Link to="/teacher/myclass/students"><i className="ri-circle-fill circle-icon w-auto" /> 학생 관리</Link></li>
                <li><Link to="/student/list"><i className="ri-circle-fill circle-icon w-auto" /> 학생 리스트</Link></li>
              </ul>
            </li>
          )}
          ...
          {/* 수업 일정 - TEACHER */}
          {has('TEACHER') && (
            <li><Link to="/teacher/schedule"><i className="ri-calendar-schedule-line" /><span>수업 일정</span></Link></li>
          )}

          {/* 학생 - STUDENT (TEACHER는 나의 학급에 학생 리스트 포함) */}
          {has('STUDENT') && (
            <li className={dc(open.student)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('student') }}>
                <i className="ri-graduation-cap-line" /><span>학생</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/student/list"><i className="ri-circle-fill circle-icon w-auto" /> 학생 리스트</Link></li>
                {/* [cheol] 학생 본인 정보 페이지 */}
                <li><Link to="/student/myinfo"><i className="ri-circle-fill circle-icon w-auto" /> 학생세부사항</Link></li>
              </ul>
            </li>
          )}
          ...
          {has('PARENT') && (
            <li className={dc(open.parent)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('parent') }}>
                <i className="ri-id-card-line" /><span>나의 자녀</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/parent/children/status"><i className="ri-circle-fill circle-icon w-auto" /> 출결 현황</Link></li>
                <li><Link to="/parent/dashboard"><i className="ri-circle-fill circle-icon w-auto" /> 성적 조회</Link></li>
              </ul>
            </li>
          )}
          ...
          {has('TEACHER') && (
            <li className={dc(open.teacher)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('teacher') }}>
                <i className="ri-user-follow-line" /><span>선생님</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/teacher/list"><i className="ri-circle-fill circle-icon w-auto" /> 교직원 목록</Link></li>
              </ul>
            </li>
          )}
          ...
          {has('TEACHER', 'ADMIN') && (
            <li className={dc(open.parentList)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('parentList') }}>
                <i className="ri-user-heart-line" /><span>학부모</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/teacher/parent/list"><i className="ri-circle-fill circle-icon w-auto" /> 학부모 목록</Link></li>
              </ul>
            </li>
          )}
          ...
          {has('TEACHER', 'ADMIN', 'STUDENT') && (
            <li className={dc(open.notice)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('notice') }}>
                <i className="ri-megaphone-line" /><span>공지사항</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/board/school-notice"><i className="ri-circle-fill circle-icon w-auto" /> 학교 공지</Link></li>
              </ul>
            </li>
          )}
          ...
          {has('STUDENT', 'TEACHER', 'ADMIN') && (
            <li className={dc(open.board)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('board') }}>
                <i className="ri-article-line" /><span>게시판</span>
              </a>
              <ul className="sidebar-submenu">
                {/* [cheol] 학생 본인 학년으로 이동, TEACHER/ADMIN은 1학년 기본 */}
                <li><Link to={role === 'STUDENT' ? `/board/grade/1` : '/board/grade/1'}><i className="ri-circle-fill circle-icon w-auto" /> 학년 게시판</Link></li>
                {has('TEACHER', 'ADMIN') && <li><Link to="/board/teacher"><i className="ri-circle-fill circle-icon w-auto" /> 교직원 게시판</Link></li>}
              </ul>
            </li>
          )}
          ...
          {has('STUDENT', 'TEACHER', 'ADMIN') && (
            <li className={dc(open.exam)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('exam') }}>
                <i className="ri-file-edit-line" /><span>과제</span>
              </a>
              <ul className="sidebar-submenu">
                {/* [cheol] 성적/시험 관련 React 페이지 */}
                <li><Link to="/exam"><i className="ri-circle-fill circle-icon w-auto" /> 성적 조회</Link></li>
                <li><Link to="/exam/schedule"><i className="ri-circle-fill circle-icon w-auto" /> 시험 일정</Link></li>
                <li><Link to="/exam/result"><i className="ri-circle-fill circle-icon w-auto" /> 성적 결과</Link></li>
              </ul>
            </li>
          )}
          ...
          {has('TEACHER', 'ADMIN') && (
            <li className={dc(open.attendance)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('attendance') }}>
                <i className="ri-calendar-check-line" /><span>출결 관리</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/attendance/student"><i className="ri-circle-fill circle-icon w-auto" /> 학생 출결</Link></li>
                <li><Link to="/attendance/teacher"><i className="ri-circle-fill circle-icon w-auto" /> 교사 출결</Link></li>
              </ul>
            </li>
          )}
          ...
          {has('PARENT', 'ADMIN') && (
            <li className={dc(open.parentNotice)}>
              ...
              <ul className="sidebar-submenu">
                <li><Link to="/board/parent-notice"><i className="ri-circle-fill circle-icon w-auto" /> 가정통신문</Link></li>
                <li><Link to="/board/school-notice"><i className="ri-circle-fill circle-icon w-auto" /> 학교 공지</Link></li>
                <li><Link to="/board/grade/1"><i className="ri-circle-fill circle-icon w-auto" /> 학급 공지</Link></li>
                <li><Link to="/school/gallery"><i className="ri-circle-fill circle-icon w-auto" /> 학교 갤러리</Link></li>
                <li><Link to="/school/schedule"><i className="ri-circle-fill circle-icon w-auto" /> 학교 일정</Link></li>
              </ul>
            </li>
          )}

          {/* [soojin] 상담 - PARENT / TEACHER */}
          {has('PARENT', 'TEACHER') && (
            <li className={dc(open.consultation)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('consultation') }}>
                <i className="ri-customer-service-2-line" /><span>상담</span>
              </a>
              <ul className="sidebar-submenu">
                {has('PARENT') && <li><Link to="/consultation/reservation"><i className="ri-circle-fill circle-icon w-auto" /> 상담 신청 예약</Link></li>}
                {has('PARENT') && <li><Link to="/consultation"><i className="ri-circle-fill circle-icon w-auto" /> 상담 예약 확인</Link></li>}
                {has('TEACHER') && <li><Link to="/consultation"><i className="ri-circle-fill circle-icon w-auto" /> 상담 목록</Link></li>}
                {has('TEACHER') && <li><Link to="/consultation/reservation"><i className="ri-circle-fill circle-icon w-auto" /> 예약 캘린더</Link></li>}
              </ul>
            </li>
          )}
          ...
          {has('PARENT', 'TEACHER', 'ADMIN') && (
            <li className={dc(open.parentBoard)}>
              <a href="#" onClick={e => { e.preventDefault(); toggle('parentBoard') }}>
                <i className="ri-edit-box-line" /><span>학부모 게시판</span>
              </a>
              <ul className="sidebar-submenu">
                <li><Link to="/board/parent-notice"><i className="ri-circle-fill circle-icon w-auto" /> 학부모 공지</Link></li>
                <li><Link to="/board/parent"><i className="ri-circle-fill circle-icon w-auto" /> 학부모 게시판</Link></li>
              </ul>
            </li>
          )}
