...
export default function ClassGoalEditor({ classroomId }: Props) {
  ...
  useEffect(() => {
    api
      .get(`/class/goal/${classroomId}?year=${year}&month=${month}`)
      .then((res) => {
  ...
  const updateItem = (idx: number, val: string) => setEditItems((prev) => prev.map((v, i) => (i === idx ? val : v)));

  const save = () => {
    if (!editGoal.trim()) return;
    setSaving(true);
    api
      .post(`/class/goal/${classroomId}?year=${year}&month=${month}`, {
        goal: editGoal.trim(),
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {editItems.map((item, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      >
                        <span
                          style={{ width: 6, height: 6, borderRadius: "50%", background: "#25A194", display: "block" }}
                        />
                      </span>
                      <input
                        type="text"
